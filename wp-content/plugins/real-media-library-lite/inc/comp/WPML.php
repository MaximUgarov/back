...
class WPML implements \MatthiasWeb\RealMediaLibrary\overrides\interfce\comp\IOverrideWPML
{
    ...
    {
        if (!$isShortcut && get_option('rml_wpml_move', '1') === '1' && \json_encode($ids) !== \json_encode($this->previousIds) && $folderId !== $this->previousFolderId) {
            global $sitepress;
            $moveToFolder = [];
    ...
    public function wpml_media_create_duplicate_attachment($post_id, $tr_id)
    {
        ...
        \MatthiasWeb\RealMediaLibrary\attachment\CountCache::getInstance()->addNewAttachment($tr_id)->resetCountCacheOnWpDie(_wp_rml_root());
    }
    /**
