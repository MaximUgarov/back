...
class ExImport implements \MatthiasWeb\RealMediaLibrary\overrides\interfce\comp\IOverrideExImport
{
    ...
    public function getFolders()
    {
        ...
        $metas = \MatthiasWeb\RealMediaLibrary\Util::getInstance()->group_by($wpdb->get_results("SELECT * FROM {$table_name}", ARRAY_A), 'realmedialibrary_id');
        ...
        // Create tree
        $tree = \MatthiasWeb\RealMediaLibrary\Util::getInstance()->buildTree($grouped, -1, 'parent', 'id', '__children');
    ...
    {
        $value = '1';
        $expireOption = new \MatthiasWeb\RealMediaLibrary\Vendor\MatthiasWeb\Utils\ExpireOption(RML_OPT_PREFIX . '_importTaxNotice', \false, 365 * \constant('DAY_IN_SECONDS'));
        $expireOption->enableTransientMigration(\MatthiasWeb\RealMediaLibrary\Vendor\MatthiasWeb\Utils\ExpireOption::TRANSIENT_MIGRATION_SITE_WIDE);
        if ($set !== null) {
