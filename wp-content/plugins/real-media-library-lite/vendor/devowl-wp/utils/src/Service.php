...
class Service
{
    ...
    const SECURITY_PLUGINS_BLOCKING_REST_API = ['better-wp-security', 'all-in-one-wp-security-and-firewall', 'sucuri-scanner', 'anti-spam', 'wp-cerber', 'wp-simple-firewall', 'wp-hide-security-enhancer', 'bulletproof-security', 'disable-json-api', 'ninjafirewall', 'hide-my-wp', 'perfmatters', 'swift-performance', 'clearfy'];
    /**
     * C'tor.
     *
    ...
    public function admin_notices()
    {
        if (!isset($GLOBALS[self::NOTICE_CORRUPT_REST_API_ID])) {
            ...
            echo \sprintf('<div id="notice-corrupt-rest-api" class="hidden notice notice-warning"><p>%s</p><ul></ul><p>%s</p></div>', \sprintf(
                // translators:
                __('One or more WordPress plugins tried to call the WordPress REST API, which failed. Most likely a security plugin%s or a web server configuration disabled the REST API. Please make sure that the following REST API namespaces are reachable to use your plugin without problems:', 'devowl-wp-utils'),
                ...
                '<a href="' . esc_attr(__('https://devowl.io/knowledge-base/i-see-only-a-loading-animation-wordpress-rest-api-not-responding/', 'devowl-wp-utils')) . '" target="_blank" rel="noreferrer">',
                '</a>'
            ));
        }
    }
    ...
    {
        $slug = $instance->getPluginConstant(\MatthiasWeb\RealMediaLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_SLUG);
