...
class Assets
{
    ...
    public function overrideLocalizeScript($context)
    {
        ...
        return ['canBeRated' => $ratingHandler->getCanBeRated(), 'rateLinks' => $ratingHandler->getLinks(), 'cross' => \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSellingHandler()->getAvailable(), 'names' => $names];
    }
    /**
    ...
    {
        $assets = $this->getFirstAssetsToEnqueueComposer();
        $enqueuePointer = \count(\MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getRatingHandler()->getCanBeRated()) > 0;
