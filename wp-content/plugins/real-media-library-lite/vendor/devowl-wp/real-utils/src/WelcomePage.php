...
class WelcomePage
{
    ...
    public function plugin_row_meta($links, $plugin)
    {
        if ($plugin === plugin_basename($this->getInitiator()->getPluginFile())) {
            ...
            $links[] = \sprintf($linkTemplate, $this->getInitiator()->getRateLink(), '_blank', __('Rate plugin', REAL_UTILS_TD));
        }
        return $links;
    }
    /**
    ...
    public function activated_plugin($plugin)
    {
        ...
        if (!$fromBulkAction && !$isWpCli && $plugin === plugin_basename($this->getInitiator()->getPluginFile())) {
            $alreadyRedirected = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::get($this->getInitiator(), \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE, \false);
            if (!$alreadyRedirected && wp_safe_redirect($this->getWelcomePageLink())) {
                \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::set($this->getInitiator(), \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE, \true);
                exit;
            }
        }
    }
    ...
    public function admin_menu()
    {
        ...
        $title = $data['Name'];
        add_submenu_page(null, $title, $title, 'install_plugins', $slug . self::PAGE_SUFFIX, [$this, 'welcome_page_output']);
    }
    /**
    ...
    <nav class="nav-tab-wrapper wp-clearfix">
        ...
        <a target="_blank" href="%rateLink" class="nav-tab">' . __('Rate plugin', REAL_UTILS_TD) . '</a>
    </nav>
    ...
    <div class="about-wrap-content">
        ...
        $heroButton = isset($heroButton) ? \sprintf('<a href="%s" class="button button-primary button-hero" style="margin-top: 40px;"><strong>%s</strong></a>', add_query_arg('feature', 'welcome-page', $heroButton[1]), $heroButton[0]) : '';
        $data = ['%slug' => $this->getInitiator()->getPluginSlug(), '%title' => $pluginData['Name'], '%version' => $pluginData['Version'], '%subtitle' => \str_replace('<cite>', \sprintf('<br /><cite>', \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getBaseAssetsUrl('devowl.io-logo.svg')), $pluginData['Description']), '%logoUrl' => $this->getInitiator()->getAssetsUrl('logo.svg'), '%supportLink' => $this->getInitiator()->getSupportLink(), '%rateLink' => $this->getInitiator()->getRateLink(), '%heroButton' => $heroButton, '%featuresHtml' => $this->transformKeyFeaturesHtml(), '%currentEmail' => esc_attr(get_userdata(get_current_user_id())->user_email), '%ourPluginsHeader' => $ourPluginsOutput['header']];
    ...
    {
        return is_admin() && isset($_GET['page']) && $_GET['page'] === $this->getInitiator()->getPluginSlug() . \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\WelcomePage::PAGE_SUFFIX;
    }
    /**
     * Create instance.
     *
