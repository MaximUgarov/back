...
class Options
{
    ...
    {
        if (\MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSellingHandler()->isAnyProInstalled() && current_user_can('install_plugins')) {
    ...
    {
        if (\MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSellingHandler()->isAnyProInstalled()) {
