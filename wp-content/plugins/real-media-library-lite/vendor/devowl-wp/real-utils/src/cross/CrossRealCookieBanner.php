...
class CrossRealCookieBanner extends \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\cross\AbstractCrossSelling
{
    ...
    {
        return ['gdpr-compliant' => ['title' => __('Is your website GDPR compliant?', REAL_UTILS_TD), 'image' => $this->getAssetsUrl(__('cookie-banner-frontend.png', REAL_UTILS_TD)), 'description' => __('Websites targeting users from the EU and setting non-essential cookies require an opt-in cookie banner. We recommend Real Cookie Banner as ePrivacy Directive and GDPR compliant solution for WordPress.', REAL_UTILS_TD), 'link' => __('https://devowl.io/go/real-cookie-banner?source=cross-rcb', REAL_UTILS_TD)]];
    }
    /**
    ...
    public function hasGdprPluginInstalled($plugins)
    {
        foreach ($plugins as $slug) {
            $file = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSellingHandler()->isInstalled($slug, \true);
            if ($file !== \false) {
