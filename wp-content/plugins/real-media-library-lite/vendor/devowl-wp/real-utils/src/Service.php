...
' . $note, 'siteHealth' => 'none', 'privacy' => \true]]);
    ...
    public function routeRatingDismiss($request)
    {
        $slug = $request->get_param('slug');
        $force = $request->get_param('force');
        $result = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getRatingHandler()->dismiss($slug, $force);
        // Currently, ignore errors
        return new \WP_REST_Response(['result' => $result]);
    }
    /**
    ...
    public function routeNewsletterSubscribe($request)
    {
        ...
        $result = wp_remote_post('https://devowl.io/wp-json/devowl-site/v1/plugin-activation-newsletter', ['body' => ['email' => $email, 'referer' => home_url(), 'slug' => $slug]]);
